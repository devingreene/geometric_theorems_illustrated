...
def redraw_triangle():
    ...
    for i in range(3):
        inner_triangle[i][0].set_data((inner_triangle_vertices[i][0],
            inner_triangle_vertices[(i+1)%3][0]),
            (inner_triangle_vertices[i][1],
                inner_triangle_vertices[(i+1)%3][1]))
...
def setup_trisectors():
    ...
    sides = [ sqrt((x[i%3]-x[(i+1)%3])**2 + (y[i%3]-y[(i+1)%3])**2)\
            for i in range(1,4)]
    angles = [ arccos((sides[(i+1)%3]**2+sides[(i+2)%3]**2 - sides[i]**2)/\
            (2*sides[(i+1)%3]*sides[(i+2)%3]))/3 \
        for i in range(3) ]
...
def setup_inner_triangle_vertices():
    return  ( point_2_point( x[0],x[2],y[0],y[2],r_d[0][0],r_d[5][0],
        r_d[0][1],r_d[5][1]),
        point_2_point( x[0],x[1],y[0],y[1],r_d[1][0],r_d[2][0],
            r_d[1][1],r_d[2][1]),
        point_2_point( x[1],x[2],y[1],y[2],r_d[3][0],r_d[4][0],
            r_d[3][1],r_d[4][1])  )
...
def redraw_trisectors():
    ...
    for i in range(6):
        iq = i//2
        iq1 =((i-1) % 6 //2  + 1) % 3
        trisector_plots[i][0].set_data((x[iq],vertices[iq1][0]),
                (y[iq],vertices[iq1][1]))
...
for i in range(6):
    iq = i//2
    iq1 = ((i-1) % 6 //2  + 1 ) % 3
    trisector_plots += (plot((x[iq],vertices[iq1][0]),
        (y[iq],vertices[iq1][1])),)
